package com.yuan.demo.app.web.utils;
...
public class FixSizeMemAppender<E> extends UnsynchronizedAppenderBase<E> {
     ...
     * 日志容器
     */
    private static final Map<String, FixSizeLog> FIX_SIZE_LOG_MAP = new TreeMap<String, FixSizeLog>(); //  用TreeMap 这样能按key来排序
    ...
    private Executor exe = Executors.newSingleThreadExecutor();//  为了解决线程安全问题，直接放到另一个线程上异步处理
        ...
        if (e instanceof LoggingEvent) {
            exe.execute(new Runnable() {
                ...
                public void run() {
                    ...
                    if (fsl == null) {
                        // synchronized (this) {
                        // 简单同步 // 为了解决线程安全问题，直接放到另一个线程上异步处理,所以也就不用同步了
